...
func createResponse(id any, result any, resultType string) mcp.JSONRPCMessage {
	// If result is a pointer to a struct with ResultType, set it
	rv := reflect.ValueOf(result)
	if rv.Kind() == reflect.Ptr {
		rv = rv.Elem()
	}
	if rv.Kind() == reflect.Struct {
		field := rv.FieldByName("ResultType")
		if field.IsValid() && field.CanSet() && field.Kind() == reflect.String {
			field.SetString(resultType)
		}
	}
...
// ListTools returns a slice of all registered tools (mcp.Tool)
