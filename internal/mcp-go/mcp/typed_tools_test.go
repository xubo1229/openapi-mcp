...
func TestTypedToolHandler(t *testing.T) {
	...
	typedHandler := func(ctx context.Context, request CallToolRequest, args HelloArgs) (*CallToolResult, error) {
		return NewToolResultText(args.Name, nil, nil, nil, "", nil), nil
	}
...
func TestTypedToolHandlerWithValidation(t *testing.T) {
	...
	typedHandler := func(ctx context.Context, request CallToolRequest, args CalculatorArgs) (*CallToolResult, error) {
		// Validate operation
		if args.Operation == "" {
			return NewToolResultError("operation is required", nil, nil, nil, "", nil), nil
		}
		...
		case "divide":
			if args.Y == 0 {
				return NewToolResultError("division by zero", nil, nil, nil, "", nil), nil
			}
			result = args.X / args.Y
		default:
			return NewToolResultError("invalid operation", nil, nil, nil, "", nil), nil
		}

		return NewToolResultText(fmt.Sprintf("%.0f", result), nil, nil, nil, "", nil), nil
	}
...
func TestTypedToolHandlerWithComplexObjects(t *testing.T) {
	...
	typedHandler := func(ctx context.Context, request CallToolRequest, profile UserProfile) (*CallToolResult, error) {
		...
		if profile.Name == "" {
			return NewToolResultError("name is required", nil, nil, nil, "", nil), nil
		}
		if profile.Email == "" {
			return NewToolResultError("email is required", nil, nil, nil, "", nil), nil
		}
		...
		return NewToolResultText(response, nil, nil, nil, "", nil), nil
	}
